...
class _VerticalClockPageState extends State<VerticalClockPage> {
  ...
  Future<void> _refreshHijriDate([DateTime? now]) async {
    final current = now ?? DateTime.now();
    final todayKey = _formatDateKey(current);
    if (_lastHijriCheckDate == todayKey) {
      return;
    }
    _lastHijriCheckDate = todayKey;
    ...
    final hijri = await _apiService.getHijriDate(current);
    if (!mounted) return;

    if (hijri != null) {
      final hijriStr = app_date_utils.DateUtils.formatHijriFromApi(hijri);
      await _storageService.cacheHijriDate(hijriStr, todayKey);
      if (!mounted) return;
      setState(() {
  ...
  void _updateTime() {
    if (mounted) {
      ...
      if (_formatDateKey(now) != _lastHijriCheckDate) {
        _refreshHijriDate(now);
      }
      setState(() {
